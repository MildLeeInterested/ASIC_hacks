 ...
 *      updates and maintenance by haiphamngoc
...
//position on LCD to display temperature value for each AC.
int ACrow[] = {0,0,0,0,1,0,1}; 
int ACcol[] = {0,2,10,18,2,0,10};
...
void setup()
{
  //Initalise LCD, display startup message and get ready to display temperature values.
  lcd.begin(CHARS, LINES);
  lcd.setLED2Pin(HIGH);//turn on backlight LED
  lcd.clear();
  lcd.setCursor(0, 0);//char, line
  lcd.print("      ASIC BUS HACK     ");
  lcd.setCursor(0, 1);
  lcd.print("      4/10/16 V1.02     ");
  delay(2000);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("1:      2:      3:      ");
...
void LCDupdate(int ACnumber)
//Displays the temperature data for selected AC.
{
  lcd.setCursor(ACcol[ACnumber],ACrow[ACnumber]);//char, line
  lcd.print(ACtemp[ACnumber] / 100);
  lcd.print(".");
  if ((ACtemp[ACnumber] % 100) < 10)
  {
     lcd.print("0");
  }
  lcd.print(ACtemp[ACnumber] % 100);
  //Display number of AC being updated.
  lcd.setCursor(23,1);
  lcd.print(ACnumber);
}
